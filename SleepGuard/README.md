...
1. [Releases](../../releases) から最新の `SleepGuard_vX.X.X.zip` をダウンロード
2. 任意のフォルダに展開
...
```bash
git clone https://github.com/morannbon/SleepGuard.git
cd SleepGuard
dotnet restore
...
### v1.0.6 (2026-03-24)
- 安定版リリース
- ウィンドウ終了時のイベント購読解除によるメモリリーク修正
- タイマー二重起動防止ガードを追加

### v1.0.5 (2026-03-24)
- 終了処理中の `ObjectDisposedException` エラーを修正
- `RequestExit` / `OnExit` でイベント購読を先行解除するよう改善

### v1.0.4 (2026-03-24)
- タイマースレッドのブロック解消（`Invoke` → `BeginInvoke`）
- グレースケールアイコン生成を `LockBits` 方式に変更（高速化）
- ログローテーションチェックを間引き処理に変更（軽量化）

### v1.0.3 (2026-03-24)
- タスクトレイアイコンをICOデザインに統一
- 非アクティブ時はグレースケール表示で状態を視覚的に区別

### v1.0.2 (2026-03-24)
- 多重起動防止（Mutex）を実装
- 常駐OFFで×ボタン押下時にプロセスが残留するバグを修正

### v1.0.1 (2026-03-23)
- コードクリーンアップ（未使用フィールド・デッドコード削除）
